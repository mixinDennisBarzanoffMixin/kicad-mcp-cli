...
def test_graph_planner_infers_ranks_rail_clusters_and_costs(tmp_path: Path) -> None:
    ...
    candidate = plan["placements"] or plan["diagnostic_candidate"]["placements"]
    ranks = {item["reference"]: item["rank"] for item in candidate}
    ...
    assert plan["cost"]["old"]["symbol_overlaps"] > plan["cost"]["new"]["symbol_overlaps"]
    ...
    if plan["status"] == "blocked":
        ...
        assert plan["diagnostic_candidate"]["usable"] is False
...
def test_fixed_reference_keeps_exact_old_center(tmp_path: Path) -> None:
    ...
    candidate = plan["placements"] or plan["diagnostic_candidate"]["placements"]
    u1 = next(item for item in candidate if item["reference"] == "U1")
...
def test_text_output_and_cli_are_explicitly_dry_run(tmp_path: Path) -> None:
    ...
    args = build_parser().parse_args(
        ["plan-schematic", "--sheet", "Logic", "--fix", "J1", "--format", "json"]
    )
...
"""
    ...
    if plan["acceptance"]["accepted"]:
        for metric in ("wire_crossings", "total_manhattan_wire_length_mm", "compactness_area_mm2"):
            assert plan["cost"]["new"][metric] <= plan["cost"]["old"][metric]
    else:
        assert plan["status"] == "blocked"
        assert plan["placements"] == []
        assert plan["wire_topology"] == []
        assert plan["acceptance"]["regressions"]
